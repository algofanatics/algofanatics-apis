...
class User {
  ...
  async get(req: Request, res: Response) {
    try {
      ...
      if (!user || user.length == 0)
        return apiResponse(
          res,
          errorResponse('send', 'User does not exist. You might need to create one.', JSON.stringify(':-(')),
          HTTP_STATUS_CODES.NOT_FOUND
        );

      return apiResponse(
        res,
        successResponse('get', getReasonPhrase(HTTP_STATUS_CODES.OK), JSON.stringify('users retrieved successfully')),
  ...
  async remove(req: Request, res: Response) {
    try {
      ...
      return apiResponse(
        res,
        successResponse('get', getReasonPhrase(HTTP_STATUS_CODES.OK), JSON.stringify('successfully deleted this user')),
        HTTP_STATUS_CODES.OK
      );
    } catch (error: any) {
      const response = error instanceof APIError ? error.message : error;
  ...
  async addEmail(req: Request, res: Response) {
    try {
      ...
      if (user) {
        return apiResponse(res, errorResponse('send', 'Email already exists', JSON.stringify(':-(')), HTTP_STATUS_CODES.CONFLICT);
      }
      ...
      return apiResponse(
        res,
        successResponse('save', getReasonPhrase(HTTP_STATUS_CODES.CREATED), JSON.stringify('success')),
