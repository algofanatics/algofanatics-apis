...
export class AuthenticationService {
    ...
    async signUp(createUserDto: CreateUserDto): Promise<ResponseDto<UserResponseDto & AuthToken>> {
        ...
        const responseData = { name, email, ...tokens }
        return new ResponseDto<UserResponseDto & AuthToken>('sign up successful', responseData, 200);
    }

    async signIn(data: AuthDto): Promise<ResponseDto<UserResponseDto & AuthToken>> {
        // Check if user exists
        ...
        const responseData = { name, email, ...tokens }
        return new ResponseDto<UserResponseDto & AuthToken>('logged in successfully', responseData, 200);
    }

    async logout(userId: string) {
        const endSession = await this.usersService.update(userId, { refreshToken: null });
